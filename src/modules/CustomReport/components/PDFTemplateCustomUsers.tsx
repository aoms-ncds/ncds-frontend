...
const PDFTemplateCustomUsers = (props:any) => {
  const columnCount = props.headers.length;
  console.log(props, 'DTA66');
        ...
        <View style={styles.tableContainer} >
          ...
          {props.rowData?.map((row: any, index: number) => (
            <View style={styles.tableRow} key={row._id}>
              ...
              {props.headers.includes('SubDivision') && (
                <>

                  <Text style={styles.tableCell}>{row.officialDetails?.divisionHistory?.[0]?.subDivision?.name}</Text>
                  <div style={styles.grid}></div>
                </>
              )}
              {props.headers.includes('Date of Birth') && (
                <>

                  <Text style={styles.tableCell}>{moment(row.dateOfBirth).format('DD/MM/YYYY')}</Text>
                  <div style={styles.grid}></div>
                </>
              )}
...
export default PDFTemplateCustomUsers;
