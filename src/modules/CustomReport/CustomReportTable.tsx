...
import { Grid, Card, Button, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, InputAdornment, TextField, Box, Container, Tooltip, Autocomplete, Chip } from '@mui/material';
...
// const getTotal = (field) => IROrder.reduce((sum, row) => sum + (row[field] || 0), 0);
...
const CustomReportTable = () => {
  ...
  return (
    <CommonPageLayout title="Custom report" momentFilter={
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        <Grid sx={{ p: 1 }}>
          ...
          <Button onClick={()=>navigate('/custom-report/custom-filter-iro')} variant='contained' type='button'>
    Filters
          </Button>
        </Grid>
        <Grid sx={{ p: 1 }}>
    ...
    Select data
      ...
      <Dialog open={open} onClose={()=>setOpen(false)} fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={() => console.log('Selected Data:', selectedData)}>
          Continue →
          </Button>
        </DialogActions>
      </Dialog>
