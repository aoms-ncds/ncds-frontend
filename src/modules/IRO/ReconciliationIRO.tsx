...
const ReconciliationIRO = () => {
  const [IReconciliation, setIReconciliation] = useState<IROrder[]>();
  useEffect(() => {
    IROServices.getreconciliation()
      .then((res) => {
        console.log(res, 'getreconciliation');
        setIReconciliation(res.data);
      })
      .catch((res) => {
        console.log(res);
      });
    ...
    <CommonPageLayout title="Internal Release Order">
      ...
      <Grid item xs={12} md={12}>
        <Card style={{ height: '75vh', width: '100%' }}>
          <DataGrid rows={IReconciliation ?? []} columns={columns} getRowId={(row) => row._id} />
        </Card>
      </Grid>
    </CommonPageLayout>
  );
