...
const ReleaseAmountDialogEdit = (props: ReleaseDialogProps) => {
  ...
  saveReleaseAmount = (e: { preventDefault: () => void }) => {// TODO: on release datagrid should updated
    ...
    IROServices.updateReleaseAmount(releaseAmount.IRO?? [], releaseAmount).then((res) => {
      enqueueSnackbar({
        message: res.message,
        variant: 'success',
      });
  ...
  return (
    ...
    <>
      <Dialog open={props.open} onClose={props.onClose} maxWidth="lg" fullWidth={true}>
        {/* <br /> */}
        <Typography sx={{ fontSize: 30, fontWeight: 600, p: 2 }}>Edit Release Amount</Typography>
        {/* <br /> */}
              ...
              {props.action == 'add' && (
                <Grid item xs={12} md={6} lg={4}>
                  <Autocomplete
                    ...
                    value={paymnetMethod?.find((method) => method.paymentMethod === releaseAmount?.modeOfPayment) || null}
                    options={paymnetMethod ?? []}
                    onChange={(_e, newValue) =>
                      setReleaseAmount((prev) => ({
                        ...prev,
                        modeOfPayment: newValue?.paymentMethod ?? '',
                      }))
                    }
...
export default ReleaseAmountDialogEdit;
