...
import ChildeSupportSignSheetStudyHelp from './components/ChildeSupportSignSheetStudyHelp';
...
const StudyHelp = () => {
  ...
  const supportEnabledChilds = childList?.filter((item) => item.studyHelp);
  ...
  console.log(supportEnabledChilds.filter((r)=>r.studyHelp), 'childList');
  type NewValType = {
  ...
  useEffect(() => {
    ...
    const tot = 0;
    childList.map((i) => {
      const tot = childList.reduce((sum, i) => sum + (i.studyHelp || 0), 0);
      setTotal(tot);
    });
  }, [childList]);
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      headerAlign: 'center',
      renderHeader: () => <b>{'Study Help'}</b>,
      valueGetter: (params) => params.row.childSupport?.studyHelp != 0 ? params.row.studyHelp : '',
    },
    {
  ...
  return (
    <CommonPageLayout title="Study Help">
      <Card
        sx={{
          width: {
            xs: '15%', // mobile
                  ...
                  <BlobProvider
                    document={
                      <ChildePDFTemplateStudyHelp
                        ...
                        data={childList.filter((e) => e.studyHelp)}
                      />
                    }
                  >
                    {({ loading, url }) =>
  ...
  &nbsp; and &nbsp;
                ...
                <BlobProvider
                  document={
                    <ChildeSupportSignSheetStudyHelp
                      frNo={requisition2?.FRno ?? ''}
                      month={getMonth()}
                      total={total}
                      data={childList.filter((e) => e.studyHelp)}
  ...
  ?
          ...
          <form onSubmit={(e) => {
            e.preventDefault();
            const tot = childList
          .filter((child) => child.studyHelp) // Filter only enabled children
