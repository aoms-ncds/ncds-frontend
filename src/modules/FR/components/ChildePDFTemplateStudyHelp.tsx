...
const ChildePDFTemplateStudyHelp = (props:{frNo:string; divisionId:string|null;data:Child[]|null; month: string | null; total:number;
}) => {
      ...
      <Page size={'A2'} style={styles.page} orientation='landscape'>
        <div>
          ...
          <Text style={styles.title}>
            {`Study Help to WrokersChildren's - ${div?.[0]}`} {props.data?.[0]?.childOf?.officialDetails?.divisionHistory?.at(-1)?.subDivision?.name ?
        ...
        <View style={styles.tableContainer} >
          <View style={styles.tableRow} key={0}>
            ...
            <div style={styles.grid}></div>
            <Text style={styles.tableHead}>Study Help</Text>
            <div style={styles.grid}></div>
          </View>
          ...
          {workers?.map((row, index) => (<>
            <View style={styles.tableRow} key={row._id}>
              ...
              <div style={styles.grid}></div>
              <Text style={styles.tableCell}>{row.childSupport?.studyHelp !== 0 ? row.studyHelp : ''}</Text>
              <div style={styles.grid}></div>
            </View>
          </>
          ))}
...
export default ChildePDFTemplateStudyHelp;
