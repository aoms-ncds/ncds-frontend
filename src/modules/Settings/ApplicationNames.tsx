...
import ApplicationNamesService from './extras/ApplicationNamesService';
...
const ApplicationNames = () => {
  ...
  const removeDepartment = (id: string) => {
    const snackbarId = enqueueSnackbar({
      message: 'Removing Application Name',
      variant: 'info',
    });
    ApplicationNamesService.delete(id)
      .then((res) => {
        if (department) {
  ...
  return (
    <>
      <CommonPageLayout title="Application names">
        <Card>
      ...
      <Dialog open={dialogAction !== false} onClose={handleClose} PaperProps={{ style: { width: '500px' } }}>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (dialogAction === 'add') {
              ApplicationNamesService.create(newDepartment).then((res) => {
              ...
              ApplicationNamesService.edit(newDepartment).then((res) => {
        ...
        >
          <DialogTitle>{dialogAction === 'add' ? 'Add' : 'Edit'} Application Name</DialogTitle>
          <DialogContent>
            <TextField
              autoFocus
              margin="dense"
              ...
              onChange={(e) => setNewDepartment((newDepartment: any) => ({ ...newDepartment, name: e.target.value }))}
              required
            />
          </DialogContent>
          <DialogActions>
      ...
      <Dialog open={confirmDelete} onClose={handleDeleteCancel} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Container>Do you want to delete this Application Name?</Container>
        </DialogContent>
        <DialogActions>
          <Button
            onClick={() => {
