...
const Department = () => {
  ...
  return (
    <>
      <CommonPageLayout title="Department">
        <Card>
      ...
      <Dialog open={dialogAction !== false} onClose={handleClose} PaperProps={{ style: { width: '500px' } }}>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (dialogAction === 'add') {
              DepartmentService.create(newDepartment).then((res) => {
              ...
              DepartmentService.edit(newDepartment).then((res) => {
        ...
        >
          <DialogTitle>{dialogAction === 'add' ? 'Add' : 'Edit'} Department</DialogTitle>
          <DialogContent>
            <TextField
              autoFocus
              margin="dense"
